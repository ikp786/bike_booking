...
class User extends CI_Controller {
	...
	public function __construct(){
		...
		public function validateCoupon($coupon_code){
			$couponResponse = array('coupon_code' => $coupon_code);
			
			$couponDetails = $this->db->select("*")->from("coupons")->where("coupon", $coupon_code)->get()->row();
			if(!empty($couponDetails)){
				if($couponDetails->qty > 0){
					...
					$couponResponse['status'] = 'valid';
					$couponResponse['coupon_amount'] = $couponDetails->coupon_amount;
				}else {
					$couponResponse['status'] = 'expired';
				}
			}else{
			    $couponResponse['status'] = 'invalid';
			}
		...
		public function cost_calculation(){
			try{
				$modelId = $_GET['m'] ?? null;	
				$cityId  = $_GET['c'] ?? null;	
				$extra_helmet_booked = $_GET['extra_helmet'] ?? 0;
				$coupon_apply = $_GET['coupon'] ?? 0;
				
				if(!$modelId || !$cityId){
					throw new Exception('Important parameters missing');
				}
			
				$product_data  = $this->db->select('model_at_branch.*, manufacturer.manufacturer_name, type.type, city.city_name, branch.branch_name, model.model_name, model.image_url', 'model_at_branch.model_id', 'model_at_branch.city_id')
				...
				$booked_type = $_SESSION['booked_type_id'] ?? 4;
				$from_datetime = isset($_SESSION["from_date"]) ? str_replace(['am','pm'], ['',''], $_SESSION["from_date"]) : date('d-m-Y H:i:s');
				$to_datetime = isset($_SESSION["to_date"]) ? str_replace(['am','pm'], ['',''], $_SESSION["to_date"]) : (new DateTime(date('d-m-Y H:i:s')))->modify('+ 1 hour');
				
				if(!$booked_type || !$from_datetime || !$to_datetime){
					throw new Exception('Booked Type or From date or To date is missing');
				}
				
				// coupon 
				$coupon_code = $_GET['coupon'] ?? 0;
				$coupon_value= 0;
				
				if($coupon_apply && $coupon_code){
					$coupon_data = $this->validateCoupon($coupon_code);
					$coupon_value = ($coupon_data['status'] == 'valid') ? $coupon_data['coupon_amount'] : 0;
					$coupon_status = $coupon_data['status'];
				}
				
				$_from_datetime = new DateTime($from_datetime);
				$_to_datetime   = new DateTime($to_datetime);
				...
				$hourly_rate = $product_data->hourly_rate;
				$daily_rate  = $product_data->perday_rate;
				$weekly_rate = $product_data->weekly_rate;
				$mothly_rate = $product_data->monthly_rate;
				
				$refundable_deposit = $product_data->deposit_amount;
				$extra_helmet_rate  = 50;
				$tax_percent		= 18;
				$minimum_hours      = 10;
				...
				if($booked_type == 4)
				{
					$total_cost = ($total_hours > $minimum_hours) ? ($total_hours * $hourly_rate) : ($minimum_hours * $hourly_rate);
				}
				
				//for daily booking
				if($booked_type == 1)
				{
					$anyExtraHour = $total_hours;
					$total_days = ($total_days != 0) ? $total_days : 1 ;
					$total_cost = ($total_days * $daily_rate) + ($anyExtraHour * $hourly_rate);
				}
				...
				if($booked_type == 2)
				{
					...
					//if _total_week is less than 7 days then make it 1 week minmum else total_weeks div by 7
					$_total_weeks = floor($total_days / 7) == 0 ? 1 : ($total_days / 7); 
					$total_weeks = floor($_total_weeks);
					
					$anyExtraDay = $total_days - ($total_weeks * 7);
					
					$total_cost = ($total_weeks * $weekly_rate) + ($anyExtraDay * $daily_rate) + ($anyExtraHour * $hourly_rate);
				}
				...
				{
					$anyExtraHour = $total_hours;
					$anyExtraDay  = $total_days;
					
					$total_cost = ($total_months * $monthly_rate) + ($anyExtraDay * $daily_rate) + ($anyExtraHour * $hourly_rate);
				}
				
				
				$taxes = round($total_cost * ($tax_percent / 100), 2);
				$total_cost_with_taxes = ( ($total_cost * ($tax_percent / 100)) + $total_cost);
				$grand_total = ($total_cost_with_taxes + $refundable_deposit) - $coupon_value;
				
				if($extra_helmet_booked){
					$grand_total += $extra_helmet_rate;
				}
				
				$totalCostData = array(
					'booking_duration'		=> $booking_duration,
					'from_date'				=> $_from_datetime->format('d:m:Y h:i:A'),
					'from_date_day'			=> $_from_datetime->format('l'),
					'from_date_day_num'		=> $_from_datetime->format('d'),
					'from_date_month'		=> $_from_datetime->format('F'),
					'from_date_month_time'	=> $_from_datetime->format('F').' '.$_from_datetime->format('h:i:A'),
					'from_date_time'		=> $_from_datetime->format('h:i:A'),
					
					'to_date'				=> $_to_datetime->format('d:m:Y h:i:A'),
					'to_date_day'			=> $_to_datetime->format('l'),
					'to_date_day_num'		=> $_to_datetime->format('d'),
					'to_date_month'			=> $_to_datetime->format('F'),
					'to_date_month_time'	=> $_to_datetime->format('F').' '.$_to_datetime->format('h:i:A'),
					'to_date_time'			=> $_to_datetime->format('h:i:A'),
					
					'refundable_deposit'	=> $refundable_deposit,
					'extra_helmet_rate'		=> $extra_helmet_rate,
					'extra_helmet_booked'	=> $extra_helmet_booked,
					...
					'extra_hours'			=> $anyExtraHour ?? 0,
					'extra_days'			=> $anyExtraDay ?? 0,
					...
					'total_cost_with_taxes'	=> $total_cost_with_taxes,
					'grand_total'			=> $grand_total,
					'taxes'					=> $taxes,
					
				);
				//echo '<pre>';
				//print_r($totalCostData);
				print(json_encode($totalCostData));
			}catch(Exception $e){
		...
		public function product_details(){
			...
			//$this->cost_calculation($data['product_data'][0]);exit;
